...
class TestSpacyTrainer:
    ...
    def mock_repository(self):
        ...
        repo.get_pending_training_data = AsyncMock(
            return_value=[Mock(**item) for item in MOCK_TRAINING_ITEMS]
        )
        ...
        # Statistik-Mocks
        repo.get_training_stats = AsyncMock(
            return_value={
    ...
    async def test_spacy_trainer_initialization(self, mock_repository, training_config):
        ...
        trainer = SpacyTrainer(repository=mock_repository, config=training_config)
    ...
    async def test_training_validation_split(self, mock_repository, training_config):
        ...
        mock_examples = [(f"Test item {i}", {"cats": {"3400": 1.0}}) for i in range(20)]
...
class TestBackendTrainingOrchestrator:
    ...
    def mock_ml_plugin(self):
        ...
        plugin.repository.get_training_stats = AsyncMock(
            return_value={
        ...
        plugin.repository.get_embedding_stats = AsyncMock(
            return_value={"unique_skr03_accounts": 18, "total_embeddings": 1200}
        )
        plugin.repository.get_latest_spacy_model = AsyncMock(
            return_value={
        ...
        plugin.get_training_readiness = AsyncMock(
            return_value={
        ...
        plugin.get_training_statistics = AsyncMock(
            return_value={"high_confidence_count": 75}
        )

        plugin.get_embedding_statistics = AsyncMock(
            return_value={"unique_skr03_accounts": 18}
        )

        plugin.start_spacy_training = AsyncMock(
            return_value={
                "success": True,
                "model_name": "test_model",
    ...
    def test_orchestrator_initialization(self, mock_ml_plugin):
        ...
        assert (
            orchestrator.default_training_config.model_name == "llkjj_skr03_classifier"
        )
    ...
    async def test_training_priority_calculation(self, mock_ml_plugin):
        ...
        priority = orchestrator._calculate_training_priority(
            high_readiness, high_stats, old_model
        )
        ...
        priority = orchestrator._calculate_training_priority(
            low_readiness, low_stats, recent_model
        )
    ...
    async def test_training_recommendations_generation(self, mock_ml_plugin):
        ...
        recommendations = orchestrator._generate_training_recommendations(
            mock_readiness, {"high_confidence_count": 20}
        )
...
class TestTrainingIntegrationE2E:
    ...
    async def test_full_training_workflow_mock(self):
        ...
        # Mock MLPlugin
        mock_plugin = Mock()
        mock_plugin.get_training_readiness = AsyncMock(
            return_value={"ready_for_training": True, "readiness_score": 0.9}
        )
        mock_plugin.start_spacy_training = AsyncMock(
            return_value={
                "success": True,
                "final_accuracy": 0.93,
        ...
        mock_plugin.get_training_statistics = AsyncMock(
            return_value={"high_confidence_count": 100}
        )
        mock_plugin.get_embedding_statistics = AsyncMock(
            return_value={"unique_skr03_accounts": 20}
        )
        ...
        orchestrator = create_training_orchestrator_for_backend(
            mock_plugin, custom_config
        )
    ...
    async def test_error_handling_in_training_pipeline(self):
        ...
        error_plugin = Mock()
        error_plugin.get_training_readiness = AsyncMock(

...
async def test_backend_services_standalone():
    ...
    try:
        ...
        embedding_health = ServiceHealthChecker.check_embedding_service(embedding_service)
...
async def test_plugin_protocols():
    ...
    try:
        ...
        from llkjj_ml_plugin_v2 import (
            MLPlugin, MLPluginConfig, 
            MLRepositoryInterface, GeminiClient, EmbeddingProvider
        )
        from src.integration.backend_services import (
            BackendGeminiService, BackendEmbeddingService
        )
        ...
        print(f"✅ EmbeddingProvider Protocol: {isinstance(embedding_service, EmbeddingProvider)}")
        
        # Mock Repository für Test
        class MockMLRepository:
            """Mock Repository für Test."""
            
            async def store_invoice_embedding(self, invoice_id, invoice_item_id, embedding_data):
                return uuid4()
            
            async def query_similar_items(self, query_text, query_embedding, limit=5, min_similarity=0.7):
            ...
            async def get_pending_training_data(self, limit=None, min_confidence_rating=3):
                return []

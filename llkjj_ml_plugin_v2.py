...
class MLRepositoryInterface(Protocol):
    ...
    async def store_invoice_embedding(
        self, invoice_id: UUID, invoice_item_id: UUID, embedding_data: "EmbeddingData"
    ) -> UUID:
    ...
    async def get_pending_training_data(
        self, limit: int | None = None, min_confidence_rating: int = 3
    ) -> list["TrainingItem"]:
    ...
    async def store_spacy_model(
        self, model_name: str, model_path: Path, version: str, metrics: dict
    ) -> UUID:
        """Speichere trainiertes spaCy-Modell."""
        ...

    async def get_latest_spacy_model(self, model_type: str) -> dict | None:
        """Hole neuestes spaCy-Modell für gegebenen Typ."""
        ...

    async def log_training_run(
        self, config: dict, metrics: dict, model_id: UUID
    ) -> UUID:
...
class TrainingFeedback(BaseModel):
    ...
    amount_context: float | None = None
    ...
    **Breaking Changes von v1.x:**
    ...
    def __init__(
        ...
        if not isinstance(repository, MLRepositoryInterface):
            raise TypeError(
                f"repository must implement MLRepositoryInterface, got {type(repository)}"
            )
        if not isinstance(gemini_client, GeminiClient):
            raise TypeError(
                f"gemini_client must implement GeminiClient, got {type(gemini_client)}"
            )
        if not isinstance(embedding_provider, EmbeddingProvider):
            raise TypeError(
                f"embedding_provider must implement EmbeddingProvider, got {type(embedding_provider)}"
            )

        logger.info("✅ MLPlugin v2.0 initialisiert (Stateless, Repository-Pattern)")

    async def process_invoice_pdf(
        self, pdf_path: Path, invoice_id: UUID
    ) -> InvoiceProcessingResult:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Verarbeitung erfolgreich: {result.items_processed} Items, "
    ...
    async def _enhance_with_rag(self, invoice_item: dict) -> dict | None:
        """
        ...
        try:
            ...
            rag_context = {
                ...
                "average_similarity": sum(
                    item.similarity_score for item in similar_items
                )
                / len(similar_items),
            }
            ...
            enhanced_result = await self.gemini_client.classify_with_rag_context(
                item_description, rag_context
            )

            if enhanced_result.get("success", False):
                logger.debug(
                    f"✅ RAG-Enhancement für: {item_description[:50]}... "
    ...
    async def _store_item_embedding(
        self, invoice_id: UUID, item_index: int, invoice_item: dict
    ) -> None:
        """Speichere Embedding für Rechnungsposition."""
        try:
            # Embedding erstellen
            content_text = (
                f"Lieferant: {invoice_item.get('supplier', '')} | "
            ...
            embedding_data = EmbeddingData(
                ...
                classification_confidence=invoice_item.get(
            ...
            await self.repository.store_invoice_embedding(
                invoice_id=invoice_id,
                invoice_item_id=UUID(
                    f"00000000-0000-0000-0000-{item_index:012d}"
                ),  # Temporary ID
    ...
    async def submit_training_feedback(
        ...
        logger.info(
            f"✅ Training-Feedback gespeichert: {original_classification} → {corrected_classification}"
        )
        return feedback_id
    ...
    async def start_spacy_training(
        ...
        try:
            ...
            trainer = SpacyTrainer(repository=self.repository, config=training_config)

            # Training durchführen
            training_metrics = await trainer.train_classification_model(model_name)

            logger.info(
                f"✅ spaCy-Training abgeschlossen: "
            ...
            return {
                ...
                "model_path": str(training_metrics.model_save_path)
                if training_metrics.model_save_path
                else None,
    ...
    async def get_training_readiness(self) -> dict:
        ...
        try:
            ...
            training_data = await self.repository.get_pending_training_data(
                limit=1, min_confidence_rating=1
            )
            ...
            readiness_checks = {
                ...
                "sufficient_high_confidence_feedback": training_stats.get(
                    "high_confidence_count", 0
                )
                >= 50,
                "diverse_supplier_feedback": training_stats.get("unique_suppliers", 0)
                >= 10,
                "recent_feedback_activity": training_stats.get(
                    "recent_feedback_count", 0
                )
                >= 20,
                "embedding_diversity": embedding_stats.get("unique_skr03_accounts", 0)
                >= 15,
            }
            ...
            return {
                ...
                "recommendation": (
                    ...
                    else "Training möglich"
                    if readiness_score >= 0.6
                    else "Mehr Training-Daten sammeln"
                ),
            }
...
def create_plugin_from_backend_registry(
    backend_registry, config: MLPluginConfig | None = None
) -> MLPlugin:
    """
    ...
    Example:
        >>> from llkjj_api.core.modules import registry
    ...
    if not gemini_client or not embedding_provider:
        raise RuntimeError(
            "Gemini Client oder Embedding Provider nicht in Registry verfügbar"
        )

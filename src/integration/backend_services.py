...
from ..gemini.direct_processor import GeminiDirectProcessor, GeminiDirectConfig
...
class BackendGeminiService:
    ...
    def __init__(
        ...
        """
        self.config = GeminiDirectConfig(
            api_key=api_key,
    ...
    async def classify_with_rag_context(self, item_description: str, context: dict) -> dict:
        ...
        """
        try:
            return await self.processor.classify_with_rag_context(item_description, context)
        except Exception as e:
    ...
    def create_from_backend_registry(backend_registry, config=None):
        ...
        try:
            ...
            if not repository_factory:
                raise RuntimeError("RepositoryFactory nicht in Backend-Registry verfügbar")

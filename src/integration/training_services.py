...
from ..training.spacy_trainer import TrainingConfig
...
class BackendTrainingOrchestrator:
    ...
    def __init__(self, ml_plugin, training_config: TrainingConfig | None = None):
        """
    ...
    async def assess_training_needs(self) -> dict[str, Any]:
        ...
        try:
            ...
            latest_model = await self.ml_plugin.repository.get_latest_spacy_model(
                "text_classifier"
            )
            ...
            assessment = {
                ...
                "priority": self._calculate_training_priority(
                    readiness, training_stats, latest_model
                ),
                "training_stats": training_stats,
                "embedding_stats": embedding_stats,
                "latest_model_info": latest_model,
                "recommendations": self._generate_training_recommendations(
                    readiness, training_stats
                ),
            }

            logger.info(
                f"📊 Training-Assessment: Readiness={assessment['readiness_score']:.2f}, "
    ...
    async def execute_training_pipeline(
        self, model_name: str | None = None, custom_config: TrainingConfig | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not training_result.get("success", False):
                logger.error(
                    f"❌ Training fehlgeschlagen: {training_result.get('error')}"
                )
            ...
            deployment_result = await self._deploy_model_if_ready(
            ...
            pipeline_result = {
                "success": True,
                "model_name": model_name,
                "pipeline_duration_minutes": training_result.get(
            ...
            logger.info(
                f"✅ Training-Pipeline erfolgreich: {model_name}, "
    ...
    async def schedule_automated_training(
        self, assessment_interval_hours: int = 24, min_readiness_threshold: float = 0.8
    ) -> dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"📅 Plane automatisches Training (Intervall: {assessment_interval_hours}h, "
            ...
            if current_readiness >= min_readiness_threshold:
                logger.info(
                    f"🎯 Readiness-Threshold erreicht ({current_readiness:.2f} >= {min_readiness_threshold}) "
                ...
                logger.info(
                    f"📊 Readiness unter Threshold ({current_readiness:.2f} < {min_readiness_threshold}) "
                    "- Training verschoben"
                )
            ...
            return {"immediate_training": False, "scheduled": False, "error": str(e)}
    ...
    def _calculate_training_priority(
        ...
        days_since_last_training = (
            latest_model.get("days_since_training", 999) if latest_model else 999
        )
        ...
        if high_confidence_feedback >= 100:
            priority_score += 2
        elif high_confidence_feedback >= 50:
            priority_score += 1

        if days_since_last_training >= 30:
            priority_score += 2
        elif days_since_last_training >= 14:
            priority_score += 1
    ...
    def _generate_training_recommendations(
        self, readiness: dict[str, Any], training_stats: dict[str, Any]
    ) -> list[str]:
        ...
        if not readiness_checks.get("sufficient_high_confidence_feedback"):
            recommendations.append(
                "Mehr User-Feedback mit hoher Confidence sammeln (Ziel: 50+)"
            )

        if not readiness_checks.get("diverse_supplier_feedback"):
            recommendations.append(
                "Feedback von mehr verschiedenen Lieferanten sammeln (Ziel: 10+)"
            )

        if not readiness_checks.get("recent_feedback_activity"):
            recommendations.append(
                "Aktuelle Feedback-Aktivität steigern (Ziel: 20+ letzte 30 Tage)"
            )

        if not readiness_checks.get("embedding_diversity"):
            recommendations.append(
                "Diversität der SKR03-Klassifizierungen erhöhen (Ziel: 15+ verschiedene)"
            )
    ...
    async def _prepare_training_data(self, config: TrainingConfig) -> dict[str, Any]:
        ...
        try:
            ...
            return {"data_preparation_successful": False, "error": str(e)}

    async def _validate_trained_model(
        self, model_name: str, training_result: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            validation_passed = (
                final_accuracy >= min_accuracy and final_f1_score >= min_f1_score
            )
            ...
            return {
                ...
                "model_quality": (
                    "excellent"
                    if final_accuracy >= 0.95 and final_f1_score >= 0.90
                    else "good"
    ...
    async def _deploy_model_if_ready(
        self, model_name: str, validation_result: dict[str, Any]
    ) -> dict[str, Any]:
...
def create_training_orchestrator_for_backend(
    ml_plugin, custom_config: TrainingConfig | None = None
):
    """
...
async def schedule_training_pipeline(
    ml_plugin, assessment_interval_hours: int = 24, min_readiness_threshold: float = 0.8
) -> dict[str, Any]:
    """
    ...
    return await orchestrator.schedule_automated_training(
        assessment_interval_hours, min_readiness_threshold
    )

...
class GeminiDirectProcessor:
    ...
    def __init__(self, config: Optional[GeminiDirectConfig] = None):
        """
    ...
    def _initialize_gemini_client(self) -> None:
        ...
        try:
            ...
            if not api_key:
                raise RuntimeError("GEMINI_API_KEY nicht verfügbar - weder in config noch environment")
            
            self.client = genai.Client(api_key=api_key)
            logger.debug("✅ Gemini Client erfolgreich initialisiert")
            
        except ImportError:
            raise RuntimeError("google-genai Package nicht installiert. Run: pip install google-genai")
    ...
    async def process_pdf_direct(self, pdf_path: Path) -> GeminiDirectResult:
        ...
        try:
            ...
            logger.info(f"✅ Gemini-Processing erfolgreich: {len(result.invoice_items)} Items, "
    ...
    def _validate_pdf(self, pdf_path: Path) -> bool:
        ...
        try:
            ...
            if file_size_mb > self.config.max_pdf_size_mb:
                logger.error(f"PDF zu groß: {file_size_mb:.1f}MB > {self.config.max_pdf_size_mb}MB")
                return False
...
"""
    ...
    async def _call_gemini_api(self, pdf_base64: str, prompt: str) -> Optional[dict]:
    ...
    async def classify_with_rag_context(self, item_description: str, rag_context: dict) -> dict:
        """

...
def _get_backend_settings():
    ...
    try:
        ...
        # Fallback für lokale Entwicklung im ML-Paket
        try:
            # Importiere die lokale Config als Fallback
            from src.settings_bridge import Config as LocalConfig

            return LocalConfig()
        except ImportError:
            # Letzter Fallback: Mock-Config für Tests
...
class ConfigBridge:
    ...
    def elektro_lieferanten(self) -> list[str]:
        if hasattr(self._ml_config, "elektro_lieferanten"):
            return self._ml_config.elektro_lieferanten
        return [
            "Rexel",
            "Conrad",
            "ELV",
